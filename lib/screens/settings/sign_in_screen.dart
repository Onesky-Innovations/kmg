...
class _SignInScreenState extends State<SignInScreen> {
  ...
  String _getTranslatedText(String key) {
    ...
    final Map<String, Map<String, String>> translations = {
      'en': {
        ...
        'reset_contact': 'Still need help? Contact Support.',
        'reset_done': 'Done',
        // Updated translation keys for new dialog
        'email_reset_sent':
            "Password reset email sent to $email. Check your inbox!",
        'email_validation': "Please enter a valid email address.",
        'sign_in_failed': "Sign in failed",
        'welcome_back': "Welcome back! 👋",
        'forgot_dialog_title': "Reset Password", // New
        'forgot_dialog_hint': "Enter your email address", // New
        'forgot_dialog_submit': "Send Reset Link", // New
        'forgot_dialog_cancel': "Cancel", // New
      },
      'ml': {
        'welcome': 'തിരികെ സ്വാഗതം!',
        'subtitle':
        ...
        'email_reset_sent':
            "പാസ്‌വേർഡ് റീസെറ്റ് ഇമെയിൽ $email-ലേക്ക് അയച്ചു. നിങ്ങളുടെ ഇൻബോക്സ് പരിശോധിക്കുക!",
        'email_validation': "സാധുവായ ഒരു ഇമെയിൽ വിലാസം നൽകുക.",
        'sign_in_failed': "സൈൻ ഇൻ പരാജയപ്പെട്ടു",
        'welcome_back': "തിരികെ സ്വാഗതം! 👋",
        'forgot_dialog_title': "പാസ്‌വേർഡ് റീസെറ്റ്", // New
        'forgot_dialog_hint': "നിങ്ങളുടെ ഇമെയിൽ വിലാസം നൽകുക", // New
        'forgot_dialog_submit': "റീസെറ്റ് ലിങ്ക് അയയ്ക്കുക", // New
        'forgot_dialog_cancel': "റദ്ദാക്കുക", // New
      },
    };
    final langKey = _isEnglish ? 'en' : 'ml';
    return translations[langKey]![key] ?? translations['en']![key]!;
  ...
  Future<void> _forgotPassword({required String resetEmail}) async {
    ...
    try {
      ...
      // Update the class-level email variable for the reset sent message
      setState(() => email = resetEmail);

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_getTranslatedText('email_reset_sent'))),
      );

      // Step 3: Show the "Need Help?" dialog immediately after success
      _showEmailHelpDialog();
    } on FirebaseAuthException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(e.message ?? "Failed to send reset email.")),
  ...
  // --- NEW: Separate Email Entry Dialog ---
  void _showForgotPasswordDialog() {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text(_getTranslatedText('forgot_dialog_title')),
          content: Form(
            key: dialogFormKey,
            child: TextFormField(
              ...
              keyboardType: TextInputType.emailAddress,
              decoration: InputDecoration(
                hintText: _getTranslatedText('forgot_dialog_hint'),
                labelText: _getTranslatedText('email_label'),
              ),
              validator: (value) {
                if (value == null || value.isEmpty || !value.contains('@')) {
                  return _getTranslatedText('email_validation');
                }
                return null;
              },
            ),
          ...
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: Text(_getTranslatedText('forgot_dialog_cancel')),
            ),
            TextButton(
              onPressed: () {
                if (dialogFormKey.currentState!.validate()) {
                  ...
                  // Call the actual password reset function
                  _forgotPassword(resetEmail: emailController.text.trim());
                }
              },
              child: Text(_getTranslatedText('forgot_dialog_submit')),
            ),
          ],
        );
      },
                ...
                children: [
                  ...
                  Form(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.end,
                      children: [
                        // --- Email Field (for normal sign-in) ---
                        TextFormField(
                          decoration: InputDecoration(
                            labelText: _getTranslatedText('email_label'),
                            prefixIcon: Icon(
                        ...
                        // --- Help and Forgot Password Buttons ---
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [

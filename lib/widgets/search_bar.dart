...
class _AnimatedSearchBarState extends State<AnimatedSearchBar> {
  ...
  Widget build(BuildContext context) {
    final isSearching = _controller.text.isNotEmpty;
    // Removed unused primaryColor variable

    return Container(
      height: 40,
      decoration: BoxDecoration(
        // FIX: Changed color to a solid, neutral color (e.g., light grey)
        // so it doesn't contrast with the parent widget's background.
        color: Theme.of(
          context,
        ).colorScheme.surface, // <--- FIX FOR TWO-COLOR ISSUE
        borderRadius: BorderRadius.circular(10),
      ),
      padding: const EdgeInsets.symmetric(horizontal: 8),
      child: Row(
        children: [
          // 1. INCLUDE WITH SEARCH ICON
          const Icon(Icons.search, color: Colors.grey, size: 20),
          const SizedBox(width: 6),
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                filled: false,
                isDense: true,
                // Added contentPadding to ensure input text aligns well
                contentPadding: const EdgeInsets.symmetric(vertical: 8),
                border: InputBorder.none,
                // 2. INCLUDE WITH SEARCH HINT TEXT ANIMATION
                hintText: _controller.text.isEmpty
                    ? "Search ${_categories[_currentIndex]}"
                    : null,
                hintStyle: const TextStyle(
